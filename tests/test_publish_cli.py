...
def test_command_uses_repository_publication_act_and_preserves_omitted_tags(
...
def test_command_fails_before_publishing_when_repository_module_is_misaligned(
    tmp_path,
):
    repo = tmp_path / "pagecraft"
    server = repo / "server"
    server.mkdir(parents=True)
    shutil.copy2(ROOT / "server" / "__init__.py", server / "__init__.py")
    shutil.copy2(ROOT / "server" / "publish.py", server / "publish.py")
    inputs = tmp_path / "inputs"
    shutil.copytree(SOURCE_ACTIVITY, inputs)
    ...
    assert result.returncode != 0
    assert "módulo de publicação desalinhado" in result.stderr
    assert not (repo / "activities" / SOURCE_ACTIVITY.name).exists()
